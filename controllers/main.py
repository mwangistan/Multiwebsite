...
class Versioning_Controller(Website):
    ...
    def change_version(self, version_id):
        website = request.env['website_version.version'].search([('id', '=', version_id)]).website_id
    ...
    def all_versions(self, view_id):
        ...
        all_websites = request.env['website'].search([('id', '>', 0)])

        website_ids = []
        for i in all_websites:
            website_ids.append(i.id)

        all_versions = []
        for i in website_ids:
            view = request.env['ir.ui.view'].search([('name', '=', 'Homepage'), ('website_id', '=', i)])[1]
            versions = Version.search([('website_id', '=', i), '|', ('view_ids.key', '=', view.key), ('view_ids.key', '=', 'website.footer_default')])
